...
<h2 align='center' style=" height: 30px; background: transparent;" id='daySelected'></h2>
<h1 style="height: 95px;font-size: 75px;" id='dateSelected'>00</h1>
<h5 style="height: 25px;" id='puncRea'>Reason: - </h5>
<h5 style="height: 25px;" id='puncPer'></h5>
...
function displayAttendance(monthCurr,yearCurr){
	...
	for(var i=0;i<tableCal.length;i++){//for table row
		...
		for(var j=0;j<tdCal.length;j++){
			...
			$.ajax({
				type:'POST',
				url:'attendancePM.php',	
				data:{date:"",empId:$(".centered")[1].innerText,name:$(".centered")[2].innerText,check:1,calDate:data,trow:i,tdata:j},
